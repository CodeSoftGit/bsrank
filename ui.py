...
class StarRatingPredictorGUI:
    ...
    def create_train_widgets(self):
        self.train_dataset_button = ttk.Button(self.train_frame, text="Select Dataset", command=self.select_dataset)
        self.train_dataset_button.pack(padx=15, pady=5)

        self.train_model_output_button = ttk.Button(self.train_frame, text="Select Output Path", command=self.select_model_output)
        self.train_model_output_button.pack(padx=15, pady=5)

        self.train_button = ttk.Button(self.train_frame, text="Train Model", command=self.train_model)
        self.train_button.pack(padx=15, pady=10)

        self.progress = ttk.Progressbar(self.train_frame, orient="horizontal", length=200, mode="determinate")
        self.progress.pack(padx=15, pady=10)

    def create_predict_widgets(self):
        # Prediction tab widgets
        self.load_model_button = ttk.Button(self.predict_frame, text="Load Model", command=self.load_model)
        self.load_model_button.pack(padx=15, pady=5)

        self.load_beatmap_button = ttk.Button(self.predict_frame, text="Load Beatmap", command=self.load_beatmap)
        ...
        self.predict_button = ttk.Button(self.predict_frame, text="Predict", command=self.predict)
    ...
    def create_settings_widgets(self):
        ...
        self.theme_combobox = ttk.Combobox(self.settings_frame, textvariable=self.theme_var)
        self.theme_combobox['values'] = self.root.get_themes()
        self.theme_combobox.pack(padx=15, pady=5)

        self.apply_theme_button = ttk.Button(self.settings_frame, text="Apply Theme", command=self.apply_theme)
        self.apply_theme_button.pack(padx=15, pady=10)

        self.clear_settings_button = ttk.Button(self.settings_frame, text="Clear Settings", command=self.clear_settings)
    ...
    def select_model_output(self):
        self.model_output_path = filedialog.asksaveasfilename(
            title="Select Model Output File", defaultextension=".pkl", filetypes=(("Pickle Files", "*.pkl"), ("All Files", "*.*")))
        if self.model_output_path:
            messagebox.showinfo("Output Path Selected", f"Output Path: {self.model_output_path}")
    ...
    def train_model(self):
        if not self.dataset_path or not self.model_output_path:
            messagebox.showerror("Error", "Please provide both dataset path and model output path.")
            return

        # Check if dataset folder has at least 1 folder and 1 .dat file
        if not any(os.path.isdir(os.path.join(self.dataset_path, f)) for f in os.listdir(self.dataset_path)):
            messagebox.showerror("Error", "Dataset folder must contain at least one subfolder.")
            return

        if not any(f.endswith('.dat') for f in os.listdir(self.dataset_path)):
            messagebox.showerror("Error", "Dataset folder must contain at least one .dat file.")
            return
